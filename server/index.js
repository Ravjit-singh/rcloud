...
// ==========================================
//      STATE TOGGLES (TRASH & STAR)
// ==========================================
app.put('/api/trash/:type/:id', authenticateToken, async (req, res) => {
    try {
        const { type, id } = req.params;
        const isTrash = req.body.isTrash ? 1 : 0;
        if (type === 'file') { await app.locals.db.run(`UPDATE files SET is_trash = ? WHERE id = ? AND user_id = ?`, [isTrash, id, req.user.id]); }
        else if (type === 'folder') { await app.locals.db.run(`UPDATE folders SET is_trash = ? WHERE id = ? AND user_id = ?`, [isTrash, id, req.user.id]); }
        res.json({ success: true });
...
app.put('/api/star/:type/:id', authenticateToken, async (req, res) => {
    try {
        const { type, id } = req.params;
        const isStarred = req.body.isStarred ? 1 : 0;
        if (type === 'file') { await app.locals.db.run(`UPDATE files SET is_starred = ? WHERE id = ? AND user_id = ?`, [isStarred, id, req.user.id]); }
        else if (type === 'folder') { await app.locals.db.run(`UPDATE folders SET is_starred = ? WHERE id = ? AND user_id = ?`, [isStarred, id, req.user.id]); }
        res.json({ success: true });
    } catch(e) { res.status(500).json({error: "Failed"}); }
});
    ...
    try {
        ...
        for (let folder of rootTrashFolders) {
            ...
            for (let f of allFiles) { 
                try { await fs.unlink(path.join(FILES_DIR, f.stored_name)); await fs.unlink(path.join(THUMBS_DIR, f.stored_name+'.webp')); } catch(e){} 
            }
        ...
        for (let f of looseFiles) { 
            try { await fs.unlink(path.join(FILES_DIR, f.stored_name)); await fs.unlink(path.join(THUMBS_DIR, f.stored_name+'.webp')); } catch(e){} 
        }
        await app.locals.db.run(`DELETE FROM files WHERE is_trash = 1 AND user_id = ?`, [req.user.id]);
        await app.locals.db.run(`DELETE FROM folders WHERE is_trash = 1 AND user_id = ?`, [req.user.id]);
        res.json({ success: true });
    } catch(e) { res.status(500).json({error: "Failed"}); }
});
...
app.get('/api/thumbnail/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (!isImage && !isVideo) return res.redirect(`/api/view/${req.params.id}`);

        const thumbPath = path.join(THUMBS_DIR, file.stored_name + '.webp');
        const originalPath = path.join(FILES_DIR, file.stored_name);

        try {
            await fs.access(thumbPath);
            return res.sendFile(thumbPath);
        } catch (e) { } // Thumb doesn't exist, try to generate it

        if (!sharp) return res.sendFile(originalPath); // Safely fallback if sharp isn't installed

        if (isImage) {
            await sharp(originalPath).resize(200, 200, { fit: 'cover' }).webp({ quality: 80 }).toFile(thumbPath);
            return res.sendFile(thumbPath);
        } else if (isVideo) {
            if (!ffmpeg) return res.sendFile(originalPath); // Safely fallback if ffmpeg isn't installed
            const tempFrame = path.join(THUMBS_DIR, file.stored_name + '.jpg');
            await new Promise((resolve, reject) => {
                ffmpeg(originalPath).on('end', resolve).on('error', reject)
                    .screenshots({ count: 1, timestamps: ['10%'], folder: THUMBS_DIR, filename: file.stored_name + '.jpg', size: '200x200' });
            });
            await sharp(tempFrame).webp({ quality: 80 }).toFile(thumbPath);
            await fs.unlink(tempFrame).catch(()=>{});
            return res.sendFile(thumbPath);
        }
    } catch (err) {
        // Ultimate Failsafe: Serve the raw file if anything breaks
        try {
            const file = await app.locals.db.get(`SELECT stored_name FROM files WHERE id = ?`, [req.params.id]);
            res.sendFile(path.join(FILES_DIR, file.stored_name));
...
app.get('/share/file/:share_id', async (req, res) => {
    try {
        ...
        if (!fileRecord || fileRecord.is_public !== 1) return res.status(403).send("Forbidden");
        res.sendFile(path.join(FILES_DIR, fileRecord.stored_name));
...
app.get('/share/folder/:share_id', async (req, res) => {
    try {
        ...
        <script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Rounded:opsz,wght,FILL,GRAD@24,400,0..1,0" /><style> .material-symbols-rounded { font-variation-settings: 'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24; } .material-symbols-rounded.filled { font-variation-settings: 'FILL' 1; } </style>
            ...
            <script>
                let rawFolders = ${JSON.stringify(folders)}; let rawFiles = ${JSON.stringify(files)};
                let viewMode = localStorage.getItem('rcloud_pub_view') || 'grid'; let sortBy = localStorage.getItem('rcloud_pub_sort') || 'name'; let sortOrder = localStorage.getItem('rcloud_pub_sortOrder') || 'asc';
                ...
                function getFileIcon(name) { const ext = name.split('.').pop().toLowerCase(); if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(ext)) return { icon: 'image', color: 'text-[#a8c7fa]' }; if (['pdf'].includes(ext)) return { icon: 'picture_as_pdf', color: 'text-red-400' }; if (['mp4', 'mkv', 'avi', 'mov'].includes(ext)) return { icon: 'movie', color: 'text-purple-400' }; if (['zip', 'rar', 'apk'].includes(ext)) return { icon: 'folder_zip', color: 'text-yellow-500' }; return { icon: 'description', color: 'text-blue-400' }; }
                function toggleView() { viewMode = viewMode === 'grid' ? 'list' : 'grid'; localStorage.setItem('rcloud_pub_view', viewMode); document.getElementById('pubViewIcon').textContent = viewMode === 'grid' ? 'view_list' : 'grid_view'; render(); }
                function setSort(by) { if (sortBy === by) sortOrder = sortOrder === 'asc' ? 'desc' : 'asc'; else { sortBy = by; sortOrder = 'asc'; } localStorage.setItem('rcloud_pub_sort', sortBy); localStorage.setItem('rcloud_pub_sortOrder', sortOrder); document.getElementById('pubSortMenu').classList.add('hidden'); render(); }
                function sortData(arr) { return arr.sort((a, b) => { let vA, vB; if (sortBy === 'name') { vA = a.name.toLowerCase(); vB = b.name.toLowerCase(); } else if (sortBy === 'date') { vA = new Date(a.date).getTime()||0; vB = new Date(b.date).getTime()||0; } else if (sortBy === 'size') { vA = a.size||0; vB = b.size||0; } if (vA < vB) return sortOrder === 'asc' ? -1 : 1; if (vA > vB) return sortOrder === 'asc' ? 1 : -1; return 0; }); }
                function render() {
                    const container = document.getElementById('publicContainer'); const sortedFolders = sortData([...rawFolders]); const sortedFiles = sortData([...rawFiles]);
                    if (sortedFolders.length === 0 && sortedFiles.length === 0) { container.innerHTML = '<p class="text-[#c4c7c5] mt-4">This folder is empty.</p>'; return; }
